// Port of Smhasher from https://golang.org/src/hash/maphash/smhasher_test.go
...
var (
	fixedSeed = uint64(time.Now().UnixNano())
)
...
func bytesHash(b []byte) uint64 {
	return Sum64(b, fixedSeed)
}

func stringHash(s string) uint64 {
	return Sum64([]byte(s), fixedSeed)
}
...
func TestSmhasherSeed(t *testing.T) {
	...
	s := "hello"
	for i := 0; i < N; i++ {
		h.addS_seed(s, uint64(i+1))
		h.addS_seed(s, uint64(i+1)<<32) // make sure high bits are used
	}
	h.check(t)
}
